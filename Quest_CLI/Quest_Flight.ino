...
#define Pump                ((one_hour * 12) / SpeedFactor)
#define PUMP_DURATION       (one_second * 60) //bogus value please test

// Add these definitions near the other timer definitions
#define EXPERIMENT_DURATION (48 * one_hour)
#define timeout   (EXPERIMENT_DURATION / SpeedFactor)
...
void Flying() {
  ...
  uint32_t mission_start_time = millis();  // Add this line to track mission start

  Serial.println("Flying NOW  -  x=abort");
  Serial.println("Terminal must be reset after abort");

  while (1) {
    // Add mission timeout check
    if ((millis() - mission_start_time) > timeout) {
      Serial.println("48hrs");
      return;
    }
    ...
    if ((millis() - Pump_Time) > Pump) {
      if (readbyteFromfram(PUMP_FLAG_ADDRESS) != ON_TRUE) {
        ...
        Serial.write(IO6, HIGH);
        Serial.write(IO5, HIGH);
        delay(PUMP_DURATION);
        Serial.write(IO6, LOW);
        Serial.write(IO5, LOW);
        
        // Start vibrational motors after pumping
    ...
    if ((millis() - WriteToBuffer_Time) > WriteToBuffer) {
      ...
      // Turn off vibrational motors before LED measurements
      ...
      // Resume vibrational motors after measurements
      digitalWrite(IO7, HIGH);
    }
  }
}
